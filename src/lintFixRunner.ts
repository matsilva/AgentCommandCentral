...
const DEFAULT_LINT_MODEL = models[4];
const DEFAULT_FIX_MODEL = models[6];
...
async function invokeOpencode(
  ...
  const baseArgs: string[] = options.args ?? DEFAULT_ARGS;
  const model = options.model ?? DEFAULT_LINT_MODEL;
  const extraArgs: string[] = options.extraArgs ?? [];
...
export async function generateLintTaskItems(
  ...
  const prompt = await generateLintTaskPrompt(stdout);
  const normalizedOutput = await invokeOpencode(prompt, {
    ...options.opencode,
    model: options.opencode?.model ?? DEFAULT_LINT_MODEL,
  });
...
async function runFixForIssue(
  ...
  const output = await invokeOpencode(prompt, {
    ...options,
    model: options.model ?? DEFAULT_FIX_MODEL,
  });
